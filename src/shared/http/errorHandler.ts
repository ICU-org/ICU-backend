...
/** Тип ошибки body-parser («entity.parse.failed», «entity.too.large») — без текста. */
const externalType = (err: unknown): string | undefined => {
  const value = typeof err === "object" && err !== null ? (err as { type?: unknown }).type : undefined;
...
export const errorHandler = (err: unknown, req: Request, res: Response, next: NextFunction) => {
  ...
  if (status && status < 500) {
    // Ни err, ни его message в лог: сообщение о битом JSON содержит фрагмент
    // тела запроса, то есть текст анонимного обращения.
    logger.warn({ status, type: externalType(err), requestId: req.id }, "Bad request");
